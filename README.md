...
[![skills.sh installs](https://skills.sh/b/skk-hub/humanizer)](https://skills.sh/skk-hub/humanizer)
...
This is [skk-hub/humanizer](https://github.com/skk-hub/humanizer), a fork of [blader/humanizer](https://github.com/blader/humanizer). Rebased 2026-09-17 on upstream [blader/humanizer](https://github.com/blader/humanizer) **v3.0.0** (was a fork of v2.9.1). Local delta:
...
```bash
git clone https://github.com/blader/humanizer.git /tmp/hum
cd /tmp/hum && git show v3.0.0:SKILL.md > /tmp/base.md && git show HEAD:SKILL.md > /tmp/theirs.md
cp SKILL.md /tmp/merged.md   # from a clone of this fork
git merge-file /tmp/merged.md /tmp/base.md /tmp/theirs.md   # exit code = conflict count
```

Local pattern numbers after §11 are shifted, so expect conflicts there. Check with `python scripts/validate-package.py` from a clone with this README and SKILL.md copied in.
...
```bash
npx skills add skk-hub/humanizer --global
```
...
This fork ships no plugin manifest; the plugin route (`/plugin marketplace add blader/humanizer`) installs upstream, not this fork.
...
| 12 | **Runs of bare "X is Y" sentences** (*weak alone*) | "The API is simple. The docs are short. The tests are fast." | Join related sentences; vary the shape |
| 13 | **Lecturing from a distance** (*weak alone*) | "People tend to...", "Nobody sets out to..." | Address the reader or use the writer's experience |

### C. Inflation and borrowed authority

| # | Pattern | Before | After |
|---|---------|--------|-------|
| 14 | **Overused AI words** | "delve... testament... landscape... showcasing" | Use plain words; the list in SKILL.md is the only vocabulary list |
...
- **3.1.1** (fork) - README install lines point at this fork (skills.sh badge, npx skills add skk-hub/humanizer); plugin route noted as upstream-only. 37 patterns total.
